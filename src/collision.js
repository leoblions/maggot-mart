export class Collision {
  ...
  entityCollideTile (entity) {
    // true is collide with solid
    // false means not blocked

    let x1 = entity.worldX
    let xh = entity.worldX + entity.width / 2
    let x2 = entity.worldX + entity.width
    let y1 = entity.worldY
    let yh = entity.worldY + entity.height / 2
    let y2 = entity.worldY + entity.height
    //debugger
    //corners
    let tlc = this.game.tilegrid.isSolidAtWorldCoord(x1, y1)
    let trc = this.game.tilegrid.isSolidAtWorldCoord(x1, y2)
